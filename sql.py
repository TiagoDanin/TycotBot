...
def inserir(table, nome, user_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("INSERT INTO {} (nome, user_id) VALUES ('{}','{}');".format(str(table).replace('-', 'T'),
                                                                                   str(nome), str(user_id)))
        conn.commit()
        return 'inserido'
    except:
...
def advertir(table, user_id):
    ...
    try:
        ...
        advs += 1
        cursor.execute("""UPDATE {} SET advs = {} WHERE user_id = {};""".format(str(table).replace('-', 'T'),
                                                                                advs, user_id))
        conn.commit()
        return 'alterado'
    except:
