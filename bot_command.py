...
try:
    ...
    print('Precisa instalar BeautifulSoup4 e schedule\npip install beautifulsoup4\npip install schedule')
...
class command_user(control, keyboard):
    def msgDia(self):
        ...
        if hj == 0:
            ...
            bot.sendMessage(self.chat_id, parse_mode='HTML',
                            text='<i>Bom dia {}!{}</i>'.format(msg[random.randint(0, len(msg) - 1)], hoje),
                            reply_markup=inst_keyboard.keyboard_sugestao())
            bot.sendVideo(self.chat_id, 'https://media.giphy.com/media/W4IY7zQdRh7Ow/giphy.gif')
        ...
        if self.chat_type == 'private':
            ...
            if(sql.procurar(self.chat_id, user_id) == 'erro ao procurar'):
                ...
                if resultado[3] == 1:
                    ...
                    return self.bot.forwardMessage(user_id, self.chat_id, msg),
                    self.bot.sendMessage(user_id,
                                         ('Mensagem enviada no grupo {}').format(self.msg['chat']['title']),
                                         reply_markup=self.keyboard_alert(self.chat_id, msg, first_name))
