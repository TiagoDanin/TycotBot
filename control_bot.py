	...
	def commands(self):
		...
		if self.chat_type == 'private':
			...
			if self.text.startswith('/start'):
				self.bot.sendMessage(self.chat_id, ('Olá, eu sou o Tycot!\nFui criado pela galera do Programeiros para te ajudar a administrar teu grupo!'))
			self.log()
		...
		try:
			...
			if self.text.startswith('/link'):
				self.bot.sendMessage(self.user_id, parse_mode='Markdown', text='[Programeiros](https://telegram.me/joinchat/AAAAAD-xLVoEzIPEWBM2Fw)')
				self.log()
	...
	###  ADMINS COMMANDS  ###
		...
		if (self.text.startswith('/unwarn')): #or (self.msg['data'] == 'removewarn'):
			...
			if self.user_id in self.adm_list:
				...
				if self.user_reply_id not in self.adm_list:
					self.bot.sendMessage(self.chat_id,'*{}* perdoado.'.format(user1), parse_mode='Markdown')
